...
def slide_inference(model, img, img_meta, rescale, args, valid_region=None, pred_cls=None):
    ...
    for h_idx in range(h_grids):
        for w_idx in range(w_grids):
            ...
            # preds += F.pad(crop_seg_logit.cpu().detach(), (int(x1), int(preds.shape[3] - x2), int(y1), int(preds.shape[2] - y2)))
            heat_preds += F.pad(heatmap.cpu().detach(), (int(x1), int(heat_preds.shape[3] - x2), int(y1), int(heat_preds.shape[2] - y2)))
            boundary_preds += F.pad(boundary.cpu().detach(), (int(x1), int(boundary_preds.shape[3] - x2), int(y1), int(boundary_preds.shape[2] - y2)))
    ...
    # preds = preds.float()
    heat_preds = heat_preds.float()
    boundary_preds = boundary_preds.float()
    if rescale:
        # preds = resize(
        #     preds,
    ...
    return heat_preds, boundary_preds