...
from models.CPFNet.resnet import resnet34
...
class CPFNet(nn.Module):
    def __init__(self, out_planes=3, ccm=True, norm_layer=nn.BatchNorm2d, is_training=True, expansion=2,
                 base_channel=32):
        ...
        self.cls_head = BaseNetHead(spatial_ch[0], out_planes, 2, is_aux=False, norm_layer=norm_layer)
        self.heatmap_head = BaseNetHead(spatial_ch[0], 1, 2, is_aux=False, norm_layer=norm_layer)
        self.boundary_head = BaseNetHead(spatial_ch[0], 1, 2, is_aux=False, norm_layer=norm_layer)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        cls_out = self.cls_head(d1)
        heat_out = self.heatmap_head(d1)
        boundary_out = self.boundary_head(d1)

        return cls_out, heat_out, boundary_out
