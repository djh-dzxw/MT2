...
class DACblock(nn.Module):
    ...
    def forward(self, x):
        dilate1_out = nn.ReLU(inplace=True)(self.dilate1(x))
        dilate2_out = nn.ReLU(inplace=True)(self.conv1x1(self.dilate2(x)))
        dilate3_out = nn.ReLU(inplace=True)(self.conv1x1(self.dilate2(self.dilate1(x))))
        dilate4_out = nn.ReLU(inplace=True)(self.conv1x1(self.dilate3(self.dilate2(self.dilate1(x)))))
...
class DACblock_with_inception(nn.Module):
    ...
    def forward(self, x):
        dilate1_out = nn.ReLU(inplace=True)(self.dilate1(x))
        dilate2_out = nn.ReLU(inplace=True)(self.dilate3(self.dilate1(x)))
        dilate_concat = nn.ReLU(inplace=True)(self.conv1x1(torch.cat([dilate1_out, dilate2_out], 1)))
        dilate3_out = nn.ReLU(inplace=True)(self.dilate1(dilate_concat))
        out = x + dilate3_out
        return out
...
class DACblock_with_inception_blocks(nn.Module):
    ...
    def forward(self, x):
        dilate1_out = nn.ReLU(inplace=True)(self.conv1x1(x))
        dilate2_out = nn.ReLU(inplace=True)(self.conv3x3(self.conv1x1(x)))
        dilate3_out = nn.ReLU(inplace=True)(self.conv5x5(self.conv1x1(x)))
...
class CE_Net_(nn.Module):
    def __init__(self, args):
        ...
        self.classifier = nn.Sequential(nn.ConvTranspose2d(filters[0], 32, 4, 2, 1),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(32, 32, 3, padding=1),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(32, self.num_classes, 3, padding=1))
        self.regresser = nn.Sequential(nn.ConvTranspose2d(filters[0], 32, 4, 2, 1),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(32, 32, 3, padding=1),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(32, 1, 3, padding=1))
        self.regresser_boundary = nn.Sequential(nn.ConvTranspose2d(filters[0], 32, 4, 2, 1),
                                                nn.ReLU(inplace=True),
                                                nn.Conv2d(32, 32, 3, padding=1),
                                                nn.ReLU(inplace=True),
                                                nn.Conv2d(32, 1, 3, padding=1))
    ...
    def forward(self, x):
        ...
        pred = self.classifier(d1)
        heatmap = self.regresser(d1)
        boundary = self.regresser_boundary(d1)

        return pred, heatmap, boundary
