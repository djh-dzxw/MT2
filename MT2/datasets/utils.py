...
def multi_scale_test_v2(img, anno, heatmap, boundary, scale=[0.5, 1.0, 1.5], crop_size=(512, 512)):
    img_list = []
    anno_list = []
    heatmap_list = []
    boundary_list = []
    ...
    for s in scale:
        ...
        heatmap_temp = cv2.resize(heatmap, (int(W * s), int(H * s)), interpolation=cv2.INTER_LINEAR)
        boundary_temp = cv2.resize(boundary, (int(W * s), int(H * s)), interpolation=cv2.INTER_LINEAR)
        h_temp, w_temp, _ = img_temp.shape
        h_pad = max(0, crop_size[0] - h_temp)
        w_pad = max(0, crop_size[1] - w_temp)
        ...
        heatmap_temp = cv2.copyMakeBorder(heatmap_temp, 0, h_pad, 0, w_pad, cv2.BORDER_CONSTANT, value=0)
        boundary_temp = cv2.copyMakeBorder(boundary_temp, 0, h_pad, 0, w_pad, cv2.BORDER_CONSTANT, value=0)
        img_list.append(img_temp)
        anno_list.append(anno_temp)
        heatmap_list.append(heatmap_temp)
        boundary_list.append(boundary_temp)
        valid_list.append(valid_region)
    return img_list, anno_list, heatmap_list, boundary_list, valid_list
