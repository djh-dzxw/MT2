...
def compute_metric(gt_dir, seg_dir, save_path):
    gt_path = gt_dir
    seg_path = seg_dir
            ...
            for i in range(n_H):
                for j in range(n_W):
                    ...
                    seg_roi = remove_boundary_cells(
                        seg_pad[roi_size * i:roi_size * (i + 1), roi_size * j:roi_size * (j + 1)])
    ...
    seg_metric_df.to_csv(join(save_path, 'seg_metric.csv'), index=False)
    print('mean F1 Score:', np.mean(seg_metric['F1_Score']))
    return np.mean(seg_metric['F1_Score'])


if __name__ == '__main__':
    gt_dir = '/home/zby/Cellseg/data/fix_boundary/gts'
    seg_dir = '/home/zby/Cellseg/workspace/results_unet50_ep500_b16_crp512_s16_reg_boundary/results_val/BUPT_MCPRL_heatmap'
    save_path = '/home/zby/Cellseg/workspace/results_unet50_ep500_b16_crp512_s16_reg_boundary'
    compute_metric(gt_dir, seg_dir, save_path)
